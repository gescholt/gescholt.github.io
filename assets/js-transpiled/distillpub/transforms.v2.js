...
return _createClass(FrontMatter,[{key:"url",get:function get(){if(this._url){return this._url;}else if(this.distillPath&&this.journal.url){return this.journal.url+'/'+this.distillPath;}else if(this.journal.url){return this.journal.url;}}// 'https://github.com/distillpub/post--augmented-rnns',
,set:function set(value){this._url=value;}},{key:"githubUrl",get:function get(){if(this.githubPath){return'https://github.com/'+this.githubPath;}else{return undefined;}}// TODO resolve differences in naming of URL/Url/url.
// 'http://distill.pub/2016/augmented-rnns/thumbnail.jpg',
},{key:"previewURL",get:function get(){return this._previewURL?this._previewURL:this.url+'/thumbnail.jpg';}// 'Thu, 08 Sep 2016 00:00:00 -0700',
,set:function set(value){this._previewURL=value;}},{key:"publishedDateRFC",get:function get(){return RFC(this.publishedDate);}// 'Thu, 08 Sep 2016 00:00:00 -0700',
},{key:"updatedDateRFC",get:function get(){return RFC(this.updatedDate);}// 2016,
},{key:"publishedYear",get:function get(){return this.publishedDate.getFullYear();}// 'Sept',
},{key:"publishedMonth",get:function get(){return months[this.publishedDate.getMonth()];}// 8,
},{key:"publishedDay",get:function get(){return this.publishedDate.getDate();}// '09',
},{key:"publishedMonthPadded",get:function get(){return zeroPad(this.publishedDate.getMonth()+1);}// '08',
},{key:"publishedDayPadded",get:function get(){return zeroPad(this.publishedDate.getDate());}},{key:"publishedISODateOnly",get:function get(){return this.publishedDate.toISOString().split('T')[0];}},{key:"volume",get:function get(){var volume=this.publishedYear-2015;if(volume<1){throw new Error('Invalid publish date detected during computing volume');}return volume;}},{key:"issue",get:function get(){return this.publishedDate.getMonth()+1;}// 'Olah & Carter',
},{key:"concatenatedAuthors",get:function get(){if(this.authors.length>2){return this.authors[0].lastName+', et al.';}else if(this.authors.length===2){return this.authors[0].lastName+' & '+this.authors[1].lastName;}else if(this.authors.length===1){return this.authors[0].lastName;}}// 'Olah, Chris and Carter, Shan',
},{key:"bibtexAuthors",get:function get(){return this.authors.map(function(author){return author.lastName+', '+author.firstName;}).join(' and ');}// 'olah2016attention'
},{key:"slug",get:function get(){var slug='';if(this.authors.length){slug+=this.authors[0].lastName.toLowerCase();slug+=this.publishedYear;slug+=this.title.split(' ')[0].toLowerCase();}return slug||'Untitled';}},{key:"bibliographyEntries",get:function get(){var _this=this;return new Map(this.citations.map(function(citationKey){var entry=_this.bibliography.get(citationKey);return[citationKey,entry];}));}},{key:"bibliography",get:function get(){return this._bibliography;},set:function set(bibliography){if(bibliography instanceof Map){this._bibliography=bibliography;}else if(_typeof(bibliography)==='object'){this._bibliography=mapFromObject(bibliography);}}},{key:"assignToObject",value:function assignToObject(target){Object.assign(target,this);target.bibliography=objectFromMap(this.bibliographyEntries);target.url=this.url;target.doi=this.doi;target.githubUrl=this.githubUrl;target.previewURL=this.previewURL;if(this.publishedDate){target.volume=this.volume;target.issue=this.issue;target.publishedDateRFC=this.publishedDateRFC;target.publishedYear=this.publishedYear;target.publishedMonth=this.publishedMonth;target.publishedDay=this.publishedDay;target.publishedMonthPadded=this.publishedMonthPadded;target.publishedDayPadded=this.publishedDayPadded;}if(this.updatedDate){target.updatedDateRFC=this.updatedDateRFC;}target.concatenatedAuthors=this.concatenatedAuthors;target.bibtexAuthors=this.bibtexAuthors;target.slug=this.slug;}}],[{key:"fromObject",value:function fromObject(source){var frontMatter=new FrontMatter();Object.assign(frontMatter,source);return frontMatter;}}]);}();// Copyright 2018 The Distill Template Authors
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
...
var _iterator=_createForOfIteratorHelper(frontMatter.authors),_step;try{for(_iterator.s();!(_step=_iterator.n()).done;){var author=_step.value;var hasOldStyle=Boolean(author.affiliation);var hasNewStyle=Boolean(author.affiliations);if(!hasOldStyle)continue;if(hasNewStyle){console.warn("Author ".concat(author.author," has both old-style (\"affiliation\" & \"affiliationURL\") and new style (\"affiliations\") affiliation information!"));}else{var newAffiliation={"name":author.affiliation};if(author.affiliationURL)newAffiliation.url=author.affiliationURL;author.affiliations=[newAffiliation];}}}catch(err){_iterator.e(err);}finally{_iterator.f();}return frontMatter;}function parseFrontmatter(element){var scriptTag=element.firstElementChild;if(scriptTag){var type=scriptTag.getAttribute('type');if(type.split('/')[1]=='json'){var content=scriptTag.textContent;var parsed=JSON.parse(content);return _moveLegacyAffiliationFormatIntoArray(parsed);}else{console.error('Distill only supports JSON frontmatter tags anymore; no more YAML.');}}else{console.error('You added a frontmatter tag but did not provide a script tag with front matter data in it. Please take a look at our templates.');}return{};}// Copyright 2018 The Distill Template Authors
...
function normalizeTag(string){return string.replace(/[\t\n ]+/g,' ').replace(/{\\["^`.'acu~Hvs]( )?([a-zA-Z])}/g,function(full,x,char){return char;}).replace(/{\\([a-zA-Z])}/g,function(full,char){return char;});}function parseBibtex(bibtex){var bibliography=new Map();var parsedEntries=bibtexParse.toJSON(bibtex);var _iterator2=_createForOfIteratorHelper(parsedEntries),_step2;try{for(_iterator2.s();!(_step2=_iterator2.n()).done;){var entry=_step2.value;// normalize tags; note entryTags is an object, not Map
for(var _i7=0,_Object$entries=Object.entries(entry.entryTags);_i7<_Object$entries.length;_i7++){var _Object$entries$_i=_slicedToArray(_Object$entries[_i7],2),key=_Object$entries$_i[0],value=_Object$entries$_i[1];entry.entryTags[key.toLowerCase()]=normalizeTag(value);}entry.entryTags.type=entry.entryType;// add to bibliography
bibliography.set(entry.citationKey,entry.entryTags);}}catch(err){_iterator2.e(err);}finally{_iterator2.f();}return bibliography;}function serializeFrontmatterToBibtex(frontMatter){return"@article{".concat(frontMatter.slug,",\n  author = {").concat(frontMatter.bibtexAuthors,"},\n  title = {").concat(frontMatter.title,"},\n  journal = {").concat(frontMatter.journal.title,"},\n  year = {").concat(frontMatter.publishedYear,"},\n  note = {").concat(frontMatter.url,"},\n  doi = {").concat(frontMatter.doi,"}\n}");}// Copyright 2018 The Distill Template Authors
...
function ExtractBibliography(dom,data){var bibliographyTag=dom.querySelector('d-bibliography');if(!bibliographyTag){console.warn('No bibliography tag found!');return;}var src=bibliographyTag.getAttribute('src');if(src){var path=data.inputDirectory+'/'+src;var text=fs.readFileSync(path,'utf-8');var bibliography=parseBibtex(text);var scriptTag=dom.createElement('script');scriptTag.type='text/json';scriptTag.textContent=JSON.stringify(_toConsumableArray(bibliography));bibliographyTag.appendChild(scriptTag);bibliographyTag.removeAttribute('src');}data.bibliography=parseBibliography(bibliographyTag);}// Copyright 2018 The Distill Template Authors
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
...
function ExtractCitations(dom,data){var citations=new Set(data.citations);var newCitations=collect_citations(dom);var _iterator5=_createForOfIteratorHelper(newCitations),_step5;try{for(_iterator5.s();!(_step5=_iterator5.n()).done;){var citation=_step5.value;citations.add(citation);}}catch(err){_iterator5.e(err);}finally{_iterator5.f();}data.citations=Array.from(citations);}// Copyright 2018 The Distill Template Authors
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
...
if(!dom.querySelector('meta[name=viewport]')){var _meta=dom.createElement('meta');_meta.setAttribute('name','viewport');_meta.setAttribute('content','width=device-width, initial-scale=1');head.appendChild(_meta);}}// Copyright 2018 The Distill Template Authors
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
...
module.exports=!require('./_fails')(function(){return Object.defineProperty({},'a',{get:function get(){return 7;}}).a!=7;});},{"./_fails":15}],13:[function(require,module,exports){var isObject=require('./_is-object'),document=require('./_global').document// in old IE typeof document.createElement is 'object'
,is=isObject(document)&&isObject(document.createElement);module.exports=function(it){return is?document.createElement(it):{};};},{"./_global":16,"./_is-object":19}],14:[function(require,module,exports){var global=require('./_global'),core=require('./_core'),ctx=require('./_ctx'),hide=require('./_hide'),PROTOTYPE='prototype';var _$export=function $export(type,name,source){var IS_FORCED=type&_$export.F,IS_GLOBAL=type&_$export.G,IS_STATIC=type&_$export.S,IS_PROTO=type&_$export.P,IS_BIND=type&_$export.B,IS_WRAP=type&_$export.W,exports=IS_GLOBAL?core:core[name]||(core[name]={}),expProto=exports[PROTOTYPE],target=IS_GLOBAL?global:IS_STATIC?global[name]:(global[name]||{})[PROTOTYPE],key,own,out;if(IS_GLOBAL)source=name;for(key in source){// contains in native
...
if(type&_$export.R&&expProto&&!expProto[key])hide(expProto,key,out);}}};// type bitmap
_$export.F=1;// forced
_$export.G=2;// global
_$export.S=4;// static
_$export.P=8;// proto
...
module.exports=_$export;},{"./_core":10,"./_ctx":11,"./_global":16,"./_hide":17}],15:[function(require,module,exports){module.exports=function(exec){try{return!!exec();}catch(e){return true;}};},{}],16:[function(require,module,exports){// https://github.com/zloirock/core-js/issues/86#issuecomment-115759028
...
if(lookingForLeft){nextIndex=text.indexOf(leftDelim,currIndex);if(nextIndex===-1){break;}finalData.push({type:"text",data:text.slice(currIndex,nextIndex)});currIndex=nextIndex;}else{nextIndex=findEndOfMath(rightDelim,text,currIndex+leftDelim.length);if(nextIndex===-1){break;}finalData.push({type:"math",data:text.slice(currIndex+leftDelim.length,nextIndex),rawData:text.slice(currIndex,nextIndex+rightDelim.length),display:display});currIndex=nextIndex+rightDelim.length;}lookingForLeft=!lookingForLeft;}finalData.push({type:"text",data:text.slice(currIndex)});}else{finalData.push(startData[i]);}}return finalData;};var splitWithDelimiters=function splitWithDelimiters(text,delimiters){var data=[{type:"text",data:text}];for(var i=0;i<delimiters.length;i++){var delimiter=delimiters[i];data=splitAtDelimiters(data,delimiter.left,delimiter.right,delimiter.display||false);}return data;};/* Note: optionsCopy is mutated by this method. If it is ever exposed in the
   ...
   */var renderMathInText=function renderMathInText(text,optionsCopy){var data=splitWithDelimiters(text,optionsCopy.delimiters);var fragment=document.createDocumentFragment();for(var i=0;i<data.length;i++){if(data[i].type==="text"){fragment.appendChild(document.createTextNode(data[i].data));}else{var tag=document.createElement("d-math");var _math=data[i].data;// Override any display mode defined in the settings with that
// defined by the text itself
optionsCopy.displayMode=data[i].display;try{tag.textContent=_math;if(optionsCopy.displayMode){tag.setAttribute("block","");}}catch(e){if(!(e instanceof katex.ParseError)){throw e;}optionsCopy.errorCallback("KaTeX auto-render: Failed to parse `"+data[i].data+"` with ",e);fragment.appendChild(document.createTextNode(data[i].rawData));continue;}fragment.appendChild(tag);}}return fragment;};var _renderElem=function renderElem(elem,optionsCopy){for(var i=0;i<elem.childNodes.length;i++){var childNode=elem.childNodes[i];if(childNode.nodeType===3){// Text node
...
var shouldRender=optionsCopy.ignoredTags.indexOf(childNode.nodeName.toLowerCase())===-1;if(shouldRender){_renderElem(childNode,optionsCopy);}}// Otherwise, it's something else, and ignore it.
...
function Mathematics(dom,data){var needsCSS=false;var body=dom.querySelector('body');if(!body){console.warn("No body tag found!");return;}if(data.katex&&data.katex.delimiters){global.document=dom;renderMathInElement(body,data.katex);}// render d-math tags
var mathTags=body.querySelectorAll('d-math');if(mathTags.length>0){needsCSS=true;console.warn("Prerendering ".concat(mathTags.length," math tags..."));var _iterator6=_createForOfIteratorHelper(mathTags),_step6;try{for(_iterator6.s();!(_step6=_iterator6.n()).done;){var mathTag=_step6.value;var localOptions={displayMode:mathTag.hasAttribute('block')};var options=Object.assign(localOptions,data.katex);var html=katex$2.renderToString(mathTag.textContent,options);var container=dom.createElement('span');container.innerHTML=html;mathTag.parentElement.insertBefore(container,mathTag);mathTag.parentElement.removeChild(mathTag);}}catch(err){_iterator6.e(err);}finally{_iterator6.f();}}if(needsCSS){var katexCSSTag='<link rel="stylesheet" href="https://distill.pub/third-party/katex/katex.min.css" crossorigin="anonymous">';dom.head.insertAdjacentHTML('beforeend',katexCSSTag);}}var favicon="\n";/*!
   * escape-html
...
function renderTOC(element,headings){var ToC="\n  <style>\n\n  d-toc {\n    contain: layout style;\n    display: block;\n  }\n\n  d-toc ul {\n    padding-left: 0;\n  }\n\n  d-toc ul > ul {\n    padding-left: 24px;\n  }\n\n  d-toc a {\n    border-bottom: none;\n    text-decoration: none;\n  }\n\n  </style>\n  <nav role=\"navigation\" class=\"table-of-contents\"></nav>\n  <h2>Table of contents</h2>\n  <ul>";var _iterator7=_createForOfIteratorHelper(headings),_step7;try{for(_iterator7.s();!(_step7=_iterator7.n()).done;){var el=_step7.value;// should element be included in TOC?
...
var _title=el.textContent;var link='#'+el.getAttribute('id');var newLine='<li>'+'<a href="'+link+'">'+_title+'</a>'+'</li>';if(el.tagName=='H3'){newLine='<ul>'+newLine+'</ul>';}else{newLine+='<br>';}ToC+=newLine;}}catch(err){_iterator7.e(err);}finally{_iterator7.f();}ToC+='</ul></nav>';element.innerHTML=ToC;}// Copyright 2018 The Distill Template Authors
...
var NBSP="\xA0";var NBSP_PUNCTUATION_START=/([«¿¡]) /g;var NBSP_PUNCTUATION_END=/ ([!?:;.,‽»])/g;text=text.replace(NBSP_PUNCTUATION_START,'$1'+NBSP);text=text.replace(NBSP_PUNCTUATION_END,NBSP+'$1');return text;}function quotes(text){text=text.replace(/(\W|^)"([^\s!?:;.,‽»])/g,"$1\u201C$2")// beginning "
...
var styles$1="\nd-citation-list {\n  contain: style;\n}\n\nd-citation-list .references {\n  grid-column: text;\n}\n\nd-citation-list .references .title {\n  font-weight: 500;\n}\n";function renderCitationList(element,entries){var dom=arguments.length>2&&arguments[2]!==undefined?arguments[2]:document;if(entries.size>0){element.style.display='';var list=element.querySelector('.references');if(list){list.innerHTML='';}else{var stylesTag=dom.createElement('style');stylesTag.innerHTML=styles$1;element.appendChild(stylesTag);var heading=dom.createElement('h3');heading.id='references';heading.textContent='References';element.appendChild(heading);list=dom.createElement('ol');list.id='references-list';list.className='references';element.appendChild(list);}var _iterator8=_createForOfIteratorHelper(entries),_step8;try{for(_iterator8.s();!(_step8=_iterator8.n()).done;){var _step8$value=_slicedToArray(_step8.value,2),key=_step8$value[0],entry=_step8$value[1];var listItem=dom.createElement('li');listItem.id=key;listItem.innerHTML=bibliography_cite(entry);list.appendChild(listItem);}}catch(err){_iterator8.e(err);}finally{_iterator8.f();}}else{element.style.display='none';}}// Copyright 2018 The Distill Template Authors
function CitationList(dom,data){var citationListTag=dom.querySelector('d-citation-list');if(citationListTag){var entries=new Map(data.citations.map(function(citationKey){return[citationKey,data.bibliography.get(citationKey)];}));renderCitationList(citationListTag,entries,dom);citationListTag.setAttribute('distill-prerendered','true');}}// Copyright 2018 The Distill Template Authors
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
...
]);var distillTransforms=new Map([['DistillHeader',DistillHeader],['DistillAppendix',DistillAppendix],['DistillFooter',DistillFooter]]);/* Exported functions */function render$2(dom,data){var verbose=arguments.length>2&&arguments[2]!==undefined?arguments[2]:true;var frontMatter;if(data instanceof FrontMatter){frontMatter=data;}else{frontMatter=FrontMatter.fromObject(data);}// first, we collect static data from the dom
var _iterator9=_createForOfIteratorHelper(extractors.entries()),_step9;try{for(_iterator9.s();!(_step9=_iterator9.n()).done;){var _step9$value=_slicedToArray(_step9.value,2),name=_step9$value[0],extract=_step9$value[1];if(verbose)console.warn('Running extractor: '+name);extract(dom,frontMatter,verbose);}// secondly we use it to transform parts of the dom
}catch(err){_iterator9.e(err);}finally{_iterator9.f();}var _iterator10=_createForOfIteratorHelper(transforms.entries()),_step10;try{for(_iterator10.s();!(_step10=_iterator10.n()).done;){var _step10$value=_slicedToArray(_step10.value,2),_name=_step10$value[0],transform=_step10$value[1];if(verbose)console.warn('Running transform: '+_name);// console.warn('Running transform: ', transform);
transform(dom,frontMatter,verbose);}}catch(err){_iterator10.e(err);}finally{_iterator10.f();}dom.body.setAttribute('distill-prerendered','');// the function calling us can now use the transformed dom and filled data object
if(data instanceof FrontMatter);else{frontMatter.assignToObject(data);}}function distillify(dom,data){var verbose=arguments.length>2&&arguments[2]!==undefined?arguments[2]:true;// thirdly, we can use these additional transforms when publishing on the Distill website
var _iterator11=_createForOfIteratorHelper(distillTransforms.entries()),_step11;try{for(_iterator11.s();!(_step11=_iterator11.n()).done;){var _step11$value=_slicedToArray(_step11.value,2),name=_step11$value[0],transform=_step11$value[1];if(verbose)console.warn('Running distillify: ',name);transform(dom,data,verbose);}}catch(err){_iterator11.e(err);}finally{_iterator11.f();}}function usesTemplateV2(dom){var tags=dom.querySelectorAll('script');var usesV2=undefined;var _iterator12=_createForOfIteratorHelper(tags),_step12;try{for(_iterator12.s();!(_step12=_iterator12.n()).done;){var tag=_step12.value;var src=tag.src;if(src.includes('template.v1.js')){usesV2=false;}else if(src.includes('template.v2.js')){usesV2=true;}else if(src.includes('template.')){throw new Error('Uses distill template, but unknown version?!');}}}catch(err){_iterator12.e(err);}finally{_iterator12.f();}if(usesV2===undefined){throw new Error('Does not seem to use Distill template at all.');}else{return usesV2;}}var testing={extractors:extractors,transforms:transforms,distillTransforms:distillTransforms};exports.FrontMatter=FrontMatter;exports.distillify=distillify;exports.render=render$2;exports.testing=testing;exports.usesTemplateV2=usesTemplateV2;Object.defineProperty(exports,'__esModule',{value:true});});